...
namespace BLL
{
    public class UserBll : BaseBll
    {
        ...
        /// <param name="pageindex"></param>
        /// <returns></returns>
        public PagedList<tbllogView> GetLogs(string name, string tel, int pageindex)
        {
            using (var db = new BFdbContext())
            {
                StringBuilder sql = new StringBuilder();
                sql.Append("SELECT t.*,u.name,u.mobile FROM tbl_log t left join tbl_users u on u.userid = t.userid where 1=1 ");

                if (!string.IsNullOrEmpty(name))
                    sql.AppendFormat(" AND u.name LIKE '%{0}%'", name);

                if (!string.IsNullOrEmpty(tel))
                    sql.AppendFormat(" AND u.mobile LIKE '%{0}%'", tel);

                return db.SqlQuery<tbllogView, DateTime?>(sql.ToString(), pageindex, p => p.createtime);
            }
        }
    }
}
