...
namespace BLL
{
    public class PublishBll : BaseBll
    {
        ...
        /// <param name="pageindex"></param>
        /// <returns></returns>
        public PagedList<tblbanner> GetBannerPics(string bannerType, int pageindex)
        {
            using (var db = new BFdbContext())
            {
                StringBuilder sql = new StringBuilder();
                sql.Append("SELECT * from tbl_banner where 1=1 ");

                if (!string.IsNullOrEmpty(bannerType))
                    sql.AppendFormat(" AND Banner_type={0} ", bannerType);

                return db.SqlQuery<tblbanner, DateTime?>(sql.ToString(), pageindex, p => p.Modifydatetime);
            }
        }
