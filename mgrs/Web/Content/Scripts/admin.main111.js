...
//新菜单根据Url决定逻辑
(function () {
    var lc=window.location.href.toLowerCase();
    var locationHref =lc;
    var loc=lc.indexOf("?");

    if(loc>0)
    {
        locationHref =lc.substring(0,loc);
    }
    ...
    $(".page-sidebar>ul>li>a").each(function () {
        ...
        var url = $(this).attr("href").toLowerCase().replace("/index","/");

        //if (locationHref.indexOf(url) > 0) {
        if (locationHref == url) {
